...
struct t_select<f_interests, m_eq>
{
    ...
    {
        auto &account_list = db.interest_account_list[std::get<std::pair<std::string_view, Account::interest_mask_t>>(value).first];
...
struct t_select<f_interests, m_contains>
{
    ...
    void operator()(DB &db, const Value &value, Handler &&handler) const
    {
        ...
        auto &account_list = db.interest_account_list[interest_list.front()];
        handler(std::make_pair(account_list.rbegin(), account_list.rend()));
    }
};
...
struct t_select<f_interests, m_any>
{
    ...
    void operator()(DB &db, const Value &value, Handler &&handler) const
    {
        using IterType = typename DB::InterestIndex::const_reverse_iterator;
        ...
        {
            auto &id_list = db.interest_account_list[interest];
